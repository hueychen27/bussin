...
export function eval_function(func: FunctionValue, args: RuntimeVal[]): RuntimeVal {
    ...
    let result: RuntimeVal | NullVal = MK_NULL();
    // Evaluate the function body line by line
    for (const stmt of func.body) {
        try {
            result = evaluate(stmt, scope);
        } catch (e) { // Only function handles propogated (rise up and shine boy) return, all else throws error
    ...
    return scope.enableReturn ? MK_NULL() : result;
}
